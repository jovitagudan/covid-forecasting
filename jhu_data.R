...
pop_recov <- merge(pop,recov_table,by=c("Country/Region",  "Province/State"))
normalized_recov = pop_recov[, 7:dim(pop_recov)[2] ] / pop_recov$Population * cases_per_pop
rownames(normalized_recov) <- trimws(paste(pop_recov$"Country/Region", pop_recov$"Province/State"),"r")
colnames(normalized_recov) <- as.Date(colnames(normalized_recov), format="%m/%d/%y")
